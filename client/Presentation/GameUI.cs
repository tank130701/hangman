...
namespace client.Presentation
{
    public class GameUI
    {
        ...
        private void StartGame(string roomId, string password, string category)
        {
            ...
            try
            {
                //_gameDriver.JoinRoom(roomId, password);
        ...
        private void PlayGame(string roomId, string category, string password)
        {
            ...
            //Console.WriteLine("Press any key to return to the main menu.");
            Console.ReadKey(true);
            Console.WriteLine("Returning to the room menu...");
            ShowRoom(_gameDriver.JoinRoom(roomId, password));
        }
        ...
        private void ShowRoom(JoinRoomResponse room)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (choice == "S" && room.Owner == _gameDriver.GetCurrentPlayerUsername())
                    {
                        StartGame(room.Id, room.Password, room.Category);
                        break;
                    }
                    else if (choice == "R" && room.RoomState == "InProgress")
                    {
                        PlayGame(room.Id, room.Password, room.Category);
                    }
                    else if (choice == "D" && room.Owner == _gameDriver.GetCurrentPlayerUsername())
                    {
                        DeleteRoom(room.Id);
                        break;
                    }
